...
Hệ thống đánh giá theo CriterionTree hoạt động như một **bộ máy đo lường ổn định** — chạy đều đặn tháng/quý, công bằng, có thể dự đoán. Đây là điểm mạnh.
...
5. **Campaign là gia vị, không phải bữa ăn chính** — lương thưởng vẫn đến từ đánh giá CriterionTree định kỳ
...
- Mỗi lời cảm ơn tự động tạo sự vụ `commendation` (đã xác nhận)
...
Lương + thưởng **theo đánh giá CriterionTree định kỳ** là nguồn thu nhập chính và bền vững. Campaign reward là **gia vị thêm** cho dịp đặc biệt.
...
| Thưởng hiệu quả | Theo đánh giá CriterionTree | Hàng tháng/quý |
...
type PeerRecognition = {
  ...
  // Tự động tạo một Event commendation tương ứng:
...
Tự động tạo event `commendation` với status `confirmed` (không cần xác nhận vì đã có proof là lời cảm ơn cụ thể).

## Tích hợp với Evaluation Framework (CriterionTree)

### Cách 1: Campaign không can thiệp vào chấm điểm

**Mặc định.** Campaign và lương thưởng đi riêng. CriterionTree không có mục "đã tham gia campaign X".
...
**Khi NV được ghi nhận** trong campaign (đoạt award, được peer recognition), hệ thống tự động tạo sự vụ `commendation` hoặc `campaign_reward` → event này có `criterion_node_id = null` (general event), ghi vào hồ sơ NV. Nếu phòng có cấu hình leaf `eval_type='event'` nhận campaign events, event sẽ tính vào điểm cây theo trọng số của leaf đó.
