...
## Nhóm endpoints: Calibration

Cơ chế phân tích 3 kỳ hiệu chuẩn liên tiếp và đề xuất điều chỉnh `scoringConfig`. Manager bấm nút → hệ thống phân tích → trình bày đề xuất → manager approve → tạo draft tree mới.

---

### POST /calibration/analyze

Phân tích phân phối điểm qua 3 kỳ calibration, sinh `CalibrationProposal` với danh sách thay đổi được đề xuất.
...
```json
{
  "department_id": "dept_delivery",
  "tree_id": "tree_abc123",
  "period_ids": ["pid_feb", "pid_mar", "pid_apr"]
}
```

`period_ids` tuỳ chọn. Nếu bỏ qua, hệ thống tự lấy 3 kỳ `mode=calibration`, `status IN (closed, finalized)` gần nhất của `department_id` dùng cùng `tree_id`.

**Validation:**
- 3 kỳ phải có `mode = "calibration"` và `status IN ("closed", "finalized")`
- 3 kỳ phải dùng cùng `criterion_tree_id`
- 3 kỳ phải liên tiếp theo thứ tự thời gian (không có kỳ nào xen giữa)
- Mỗi kỳ phải có ít nhất 3 NV với WorkLog — đủ dữ liệu thống kê

**Response 201:**

```json
{
  "proposal_id": "prop_20260423_delivery",
  "tree_id": "tree_abc123",
  "periods_analyzed": ["pid_feb", "pid_mar", "pid_apr"],
  "generated_at": "2026-04-23T10:00:00Z",
  "status": "pending",
  "summary": "Phát hiện 3 leaf cần điều chỉnh: 2 target quá thấp, 1 target quá cao.",
  "changes": [
    {
      "change_id": "chg_001",
      "node_id": "node_trips",
      "node_name": "Số chuyến giao",
      "field": "target_points",
      "current_value": 150,
      "suggested_value": 200,
      "confidence": 1.0,
      "trend": "stable",
      "reasoning": "3/3 kỳ có >80% NV đạt ≥90 (trung bình 94%). Đề xuất tăng target +33% để phân biệt tốt với xuất sắc."
    },
    {
      "change_id": "chg_002",
      "node_id": "node_ontime",
      "node_name": "Tỷ lệ đúng hạn",
      "field": "target_points",
      "current_value": 100,
      "suggested_value": 75,
      "confidence": 0.67,
      "trend": "improving",
      "reasoning": "2/3 kỳ có >50% NV dưới 50. Kỳ gần nhất có cải thiện (trend: improving) — đề xuất giảm nhẹ -25%."
    }
  ]
}
```
...
| error_code | HTTP | Điều kiện |
|-----------|------|-----------|
| `INSUFFICIENT_CALIBRATION_PERIODS` | 422 | Không đủ 3 kỳ calibration đã đóng |
| `MIXED_TREE_VERSIONS` | 422 | 3 kỳ dùng tree version khác nhau |
| `NON_CONSECUTIVE_PERIODS` | 422 | Các kỳ không liên tiếp |
| `INSUFFICIENT_DATA` | 422 | Một kỳ có <3 NV với WorkLog |

---

### GET /calibration/proposals/{id}

Lấy chi tiết đề xuất hiệu chuẩn đã tạo.
...
### POST /calibration/proposals/{id}/accept

Manager review và chấp nhận toàn bộ hoặc một phần đề xuất. Hệ thống clone tree hiện tại, áp dụng các thay đổi được chấp nhận, tạo draft mới.

**Request body:**

```json
{
  "accepted_change_ids": ["chg_001", "chg_002"],
  "overrides": {
    "chg_001": { "suggested_value": 180 }
  },
  "calibration_notes": "Điều chỉnh dựa trên phân tích T2–T4/2026. chg_002 giảm ít hơn đề xuất vì team đang cải thiện."
}
```

- `accepted_change_ids`: danh sách `change_id` được chấp nhận. Có thể chấp nhận một phần.
- `overrides`: Manager có thể điều chỉnh `suggested_value` trước khi apply.
- `calibration_notes`: Ghi chú lý do — lưu vào `CriterionTree.calibration_notes` của draft mới.

**Response 201:**

```json
{
  "draft_tree_id": "tree_v2_draft",
  "version": 2,
  "status": "draft",
  "applied_changes": 2,
  "skipped_changes": 0,
  "changes_applied": [
    { "node_id": "node_trips", "field": "target_points", "old": 150, "new": 180 },
    { "node_id": "node_ontime", "field": "target_points", "old": 100, "new": 75 }
  ]
}
```

Tree mới là `draft` — chưa active. Manager cần `POST /criterion-trees/{draft_tree_id}/publish` để kích hoạt.

**Lỗi:**

| error_code | HTTP | Điều kiện |
|-----------|------|-----------|
| `PROPOSAL_ALREADY_ACCEPTED` | 409 | Đề xuất đã được accept trước đó |
| `PROPOSAL_EXPIRED` | 422 | Đề xuất đã hết hạn (>30 ngày) |
| `NO_CHANGES_ACCEPTED` | 422 | `accepted_change_ids` rỗng |

---

### POST /calibration/proposals/{id}/reject

Manager từ chối toàn bộ đề xuất (không tạo draft).

**Request body:** `{ "reason": "Team đang trong giai đoạn đặc biệt — không hiệu chỉnh kỳ này" }`

**Response 200:** `{ "proposal_id": "...", "status": "rejected" }`
...
| `INSUFFICIENT_CALIBRATION_PERIODS` | 422 | Không đủ 3 kỳ calibration đã đóng dùng cùng tree version |
| `MIXED_TREE_VERSIONS` | 422 | 3 kỳ được chỉ định dùng các tree version khác nhau |
| `NON_CONSECUTIVE_PERIODS` | 422 | Các kỳ không liên tiếp theo thứ tự thời gian |
| `PROPOSAL_ALREADY_ACCEPTED` | 409 | Proposal đã được accept hoặc reject trước đó |
| `PROPOSAL_EXPIRED` | 422 | Proposal đã quá 30 ngày kể từ khi tạo |
| `NO_CHANGES_ACCEPTED` | 422 | `accepted_change_ids` rỗng — phải chấp nhận ít nhất 1 thay đổi |
